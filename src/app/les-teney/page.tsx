...
export default function GardenPublicPage() {
  ...
  const officialBossMap = useMemo(
    () =>
      new Map<string, BossMarker>(
        ((officialLayers.bosses ?? []) as BossMarker[]).map((item) => [item.coord, item])
      ),
    []
  );
  ...
  const officialBattleMap = useMemo(
    () =>
      new Map<string, OfficialPoint>(
        ((officialLayers.battleMarkers ?? []) as OfficialPoint[]).map((item) => [
          item.coord,
          item,
        ])
      ),
  ...
  const officialMarkerMap = useMemo(
    () =>
      new Map<string, OfficialPoint>(
        ((officialLayers.customMarkers ?? []) as OfficialPoint[]).map((item) => [
          item.coord,
          item,
        ])
      ),
  ...
  const adjutants = useMemo(
    () =>
      ((officialLayers.bosses ?? []) as BossMarker[]).filter(

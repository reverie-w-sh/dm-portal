...
export default function PersonalItemsClient({ data, directory }: { data: { updatedAt?: string; items: Item[] }; directory: Record<string, Player> }) {
  ...
  return <main className={styles.page}>
    <section className={styles.shell}>
      <header className={styles.hero}><div className={styles.heroIcon}>⚔️</div><div><p className={styles.eyebrow}>Уникальные сокровища Древнего Мира</p><h1>Именные вещи</h1><p>Все персональные предметы собраны не по страницам библиотеки, а по их владельцам.</p></div></header>
      <div className={styles.stats}><span><b>{data.items.length}</b> вещей</span><span><b>{groups.length}</b> владельцев</span><span>Стоимость не показываем — она давно неактуальна</span></div>

...
export default function PersonalItemsClient({ data, directory }: { data: { updatedAt?: string; items: Item[] }; directory: Record<string, Player> }) {
  ...
  const groups = useMemo(() => {
    ...
    return [...map.entries()].sort((a, b) => {
      if (sort === "owner") return a[0].localeCompare(b[0], "ru");
      const difference = sort === "count-desc"
        ? b[1].length - a[1].length
        : a[1].length - b[1].length;
      return difference || a[0].localeCompare(b[0], "ru");
    });
  ...
  return <main className={styles.page}>
    <section className={styles.shell}>
      ...
      <div className={styles.controls}><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Найти персонажа или вещь…"/><div><button
            ...
            </button><button className={sort==="owner"?styles.active:""} onClick={()=>setSort("owner")}>По нику</button></div></div>

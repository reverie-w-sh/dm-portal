...
export default function GardenNightmaresPage() {
  ...
  useEffect(() => {
    try {
      const storedRoute = localStorage.getItem("malachite-route");
      const storedDanger = localStorage.getItem("malachite-danger");
      const storedMarkers = localStorage.getItem("malachite-markers");
      const storedBattles = localStorage.getItem("malachite-battles");
  ...
  useEffect(() => {
    localStorage.setItem("malachite-route", JSON.stringify(route));
  }, [route]);

  useEffect(() => {
    localStorage.setItem("malachite-danger", JSON.stringify(dangerCells));
  }, [dangerCells]);

  useEffect(() => {
    localStorage.setItem("malachite-markers", JSON.stringify(customMarkers));
  }, [customMarkers]);

  useEffect(() => {
    localStorage.setItem("malachite-battles", JSON.stringify(battleMarkers));
  }, [battleMarkers]);

  ...
  return <section className={styles.planner} aria-labelledby="planner-title">
    ...
    {mode === "patch" && <div className={styles.workArea}>
      ...
      <div className={styles.fields}>
        ...
        <label>2. Уровень заплатки<select value={patchLevel} disabled={!slot} onChange={e => setPatchLevel(e.target.value)}><option value="">Выбери уровень</option>{patches.map(p => <option key={p.level} value={p.level}>{p.level}-й уровень ({p.rune.toLowerCase()})</option>)}</select></label>
